...
function ChecksumView({ tool }: ChecksumViewProps) {
  ...
  const dropZoneClass = `flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-10 text-center transition text-slate-400 ${dragActive ? 'border-blue-400/70 bg-blue-600/20 text-slate-200' : 'border-white/20 hover:border-white/30 hover:bg-white/5'}`
  ...
  return (
    <div className="mx-auto max-w-3xl rounded-2xl border p-10 text-left border-white/10 bg-white/5 backdrop-blur shadow-none">
      ...
      {errorMessage && <div className="mt-5 rounded-xl border px-4 py-3 text-sm border-red-400/50 bg-red-900/25 text-red-200">{errorMessage}</div>}
      ...
      <form className="mt-8 flex flex-col gap-5" onSubmit={handleCompute}>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              <input
                ...
                className="w-full rounded-lg border px-3 py-2 pr-24 text-sm border-slate-800 bg-slate-900/70 text-white"
              />
              <button
                type="button"
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg border px-2.5 py-1 text-[0.7rem] font-semibold border-blue-400/40 bg-blue-600/20 text-blue-200"
        ...
        {(showGpgPanel || gpgKeyFile) && (
          <div className="rounded-xl border p-4 border-white/10 bg-slate-900/50">
            <div className="grid gap-3 md:grid-cols-[minmax(90px,auto)_1fr_auto] items-center">
              ...
              <button type="button" className="rounded-lg border px-3 py-1.5 text-xs font-semibold border-slate-400/40 bg-slate-400/15 text-slate-200 hover:bg-slate-400/25" onClick={handleGpgKeyPick}>
                Choose key
              </button>
            </div>
            {gpgFetchStatus !== 'idle' && (
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-[minmax(90px,auto)_1fr_auto] items-center">
              ...
              <button type="button" className="rounded-lg border px-3 py-1.5 text-xs font-semibold border-slate-400/40 bg-slate-400/15 text-slate-200 hover:bg-slate-400/25" onClick={handleGpgSignaturePick}>
                Choose signature
              </button>
              <input
                type="file"
            ...
            {gpgError && <div className="mt-3 rounded-xl border px-4 py-3 text-sm border-red-400/50 bg-red-900/25 text-red-200">{gpgError}</div>}
        ...
        {computedHash && (
          <div className="rounded-xl border p-4 border-slate-400/20 bg-slate-900/50">
