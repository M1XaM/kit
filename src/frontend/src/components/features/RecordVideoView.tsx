...
function RecordVideoView({ tool }) {
  ...
  return (
    <div className="mx-auto max-w-4xl rounded-2xl border p-10 text-left border-slate-300 bg-white shadow-md dark:border-white/10 dark:bg-white/5 dark:backdrop-blur dark:shadow-none">
      ...
      {warning ? (
        <div className="mb-4 rounded-xl border px-4 py-3 text-sm border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-400/50 dark:bg-amber-900/25 dark:text-amber-200">{warning}</div>
      ) : null}

      <div className="grid gap-4 rounded-2xl border p-5 border-slate-300 bg-slate-50 dark:border-white/10 dark:bg-slate-950/70">
        <div className="overflow-hidden rounded-xl border border-slate-300 bg-black dark:border-white/10">
          <video
            ref={previewRef}
          ...
          />
          {!isLive ? (
            <div className="px-4 py-3 text-center text-xs text-slate-500 dark:text-slate-400">Live camera preview appears here while recording.</div>
          ) : null}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button className="rounded-lg bg-red-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400 dark:disabled:bg-slate-700 dark:disabled:text-slate-400" type="button" onClick={startRecording} disabled={isLive}>
            Start recording
          </button>
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400">Camera, microphone and system audio stay on your device. No uploads are used.</div>
      </div>
